...
-- MAGIC %md
-- MAGIC Run the cell below and confirm each source volume (for **orders**, **status** and **customers**) contains a single JSON file.

-- COMMAND ----------

-- MAGIC %python
-- MAGIC spark.sql(f'LIST "{DA.paths.working_dir}/orders"').display()
-- MAGIC spark.sql(f'LIST "{DA.paths.working_dir}/status"').display()
-- MAGIC spark.sql(f'LIST "{DA.paths.working_dir}/customers"').display()
...
-- MAGIC
-- MAGIC     While the pipeline is running explore the code in the **customers_pipeline.sql** for the new customers flow.
-- MAGIC
...
-- MAGIC %md
-- MAGIC 2. Run the cell below to programmatically view the files in your `/Volumes/dbacademy/ops/labuser-name/customers` volume. 
-- MAGIC
