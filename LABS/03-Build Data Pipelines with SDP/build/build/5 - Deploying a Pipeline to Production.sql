...
-- MAGIC
-- MAGIC    e. In the new tab you should see three folders: 
-- MAGIC       - **explorations**, 
-- MAGIC       - **orders**
-- MAGIC       - **status** 
...
-- MAGIC
-- MAGIC 1. In the your **Spark Declarative Pipeline** select the **orders** folder. 
-- MAGIC
...
-- MAGIC %md
-- MAGIC ### E1. Open the `status_pipeline.sql` file
-- MAGIC 1. In your Spark Declarative Pipeline editor open the **status/status_pipeline.sql** file. 
-- MAGIC
-- MAGIC 2. This files processes new data and adds it to the pipeline for order **status**
...
-- MAGIC
-- MAGIC Use the code in `status/status_pipeline.sql`, section `A. Bronze Table Creation`, to understand how the Bronze streaming table is created and how data quality is enforced.
-- MAGIC
-- MAGIC 1. This statement creates the streaming table **1_bronze_db.status_bronze_demo5** by ingesting raw JSON files from your lab volume path: `/Volumes/dbacademy/ops/your-lab-user/status/`
-- MAGIC
...
-- MAGIC
-- MAGIC    Learn more:
-- MAGIC    - Pipeline table properties: https://docs.databricks.com/aws/en/ldp/properties#pipeline-table-properties
-- MAGIC
-- MAGIC 4. The `${source}` variable in the `FROM` clause is used to dynamically reference your specific volume location using the SDP configuration parameter.
...
-- MAGIC
-- MAGIC Use the code in `status/status_pipeline.sql`, section `B. Bronze -> Silver`, to understand how the Silver streaming table is created and how data quality is enforced.
-- MAGIC
...
-- MAGIC ---
-- MAGIC
-- MAGIC Use the code in `status/status_pipeline.sql`, section `C. Use a Materialized View to Join Two Streaming Tables`, to understand how the Gold materialized view is created.
-- MAGIC
...
-- MAGIC
-- MAGIC Use the code in `status/status_pipeline.sql`, section `D. Create Gold Materialized Views for Cancelled and Delivered Orders`, to understand how the final Gold views are created.
-- MAGIC
...
-- MAGIC
-- MAGIC 1. Complete the following steps to in your **Lakeflow Editor** to configure your Spark Declarative Pipeline for production:
-- MAGIC
...
-- MAGIC %md
-- MAGIC 2. After the pipeline has completed it's first run, complete the following:
-- MAGIC
...
-- MAGIC #### Checkpoint
-- MAGIC
-- MAGIC   ![Final Demo 5 Pipeline](./Includes/images/demo5_pipeline_image_run1.png)
...
-- MAGIC 1. Run the cell below to add **4** more JSON files to your volumes to simulate new files be landed into cloud storage:
